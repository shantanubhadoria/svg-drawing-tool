...
;(function( $ ){
	...
	$.fn.ruler = function(options) {
        var rulerThickness = 18;
    	
		var defaults = {
			vRuleSize: rulerThickness,
			hRuleSize: rulerThickness,
			showCrosshair : true,
			showMousePos: true
		};//defaults
		...
		if (!Modernizr.touch) {
			...
			if (settings.showCrosshair || settings.showMousePos) {
				$(this).mousemove(function(e) {
					...
					if (settings.showMousePos) {
						$('.mousePosBox').html("x:" + (e.pageX-settings.vRuleSize-$(this).offset().left) + ", y:" + (e.pageY-settings.hRuleSize-$(this).offset().top) ).css({
		...
		$(window).resize(function(e){
			...
			while ( tickLabelPos <= $hRule.width() ) {
				...
				} else if ((( tickLabelPos - settings.vRuleSize ) %50 ) == 0 ) {
					newTickLabel = "<div class='tickMajorPlus'></div>";
					$(newTickLabel).css( "left", tickLabelPos+"px" ).appendTo($hRule);
			...
			while (tickLabelPos <= $vRule.height()) {
				if ((( tickLabelPos - settings.hRuleSize ) %100 ) == 0) {
					newTickLabel = "<div class='tickLabel'><span style='margin-right:" + settings.hRuleSize + "px;'>" + ( tickLabelPos - settings.hRuleSize ) + "&nbsp;</span></div>";
					$(newTickLabel).css( "top", tickLabelPos+"px" ).appendTo($vRule);
				} else if (((tickLabelPos - settings.hRuleSize)%50) == 0) {
					newTickLabel = "<div class='tickMajorPlus'></div>";
					$(newTickLabel).css( "top", tickLabelPos+"px" ).appendTo($vRule);
